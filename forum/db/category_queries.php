...
{
    $query = "
    SELECT q.id, q.title, q.author_id, u.username, c.name, q.created_on, COUNT(a.id) AS answers_count
    FROM questions AS q
    INNER JOIN users u on q.author_id = u.id 
    INNER JOIN categories c on q.category_id = c.id
    LEFT JOIN answers a on q.id = a.question_id
    WHERE c.id =?
    GROUP BY q.id, q.title, q.author_id, u.username,c.name, q.created_on
...
function getAllCategories(PDO $db): array
{
    $query = "
        ...
        LEFT JOIN questions AS q ON c.id = q.category_id
